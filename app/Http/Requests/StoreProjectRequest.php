...
class StoreProjectRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'budget_initial' => ['nullable', 'string'], // Will be normalized in controller
            ...
            'contractualisation_document' => [
                'nullable',
                'file',
                'mimes:pdf,doc,docx,zip,xls,xlsx',
                'max:51200'
            ],
            ...
            // Salesforce fields (conditional validation)
            'sf_opportunity_id' => ['nullable', 'string'],
            'sf_opportunity_name' => ['nullable', 'string'],
            'sf_opportunity_stage' => ['nullable', 'string'],
            'sf_opportunity_amount' => ['nullable', 'string'],

            // Dynamic arrays - Actions
            'actions' => ['nullable', 'array', 'max:100'], // Limit to 100 actions
            ...
            'livrable_document' => ['nullable', 'array'],
            ...
            // Dynamic arrays - Issues (Enjeux/Contraintes/Risques/REX)
        ...
        // If external project, Salesforce is required
        if ($this->input('type_projet') === 'Externe') {
            $rules['sf_opportunity_id'][] = 'required';
            $rules['sf_opportunity_name'][] = 'required';
        }
    ...
    public function messages(): array
    {
        return [
            ...
            // Salesforce
            'sf_opportunity_id.required' => 'Veuillez sélectionner une opportunité Salesforce pour un projet de type Externe.',
            'sf_opportunity_name.required' => 'Le nom de l\'opportunité Salesforce est obligatoire pour un projet externe.',
            ...
            if (!$policy->validateManagerFiliale($user, $this->all())) {
                ...
                if (!$userFiliale) {
                    $validator->errors()->add(
                        'filiale_executant',
                        'Votre compte n\'a pas de filiale associée. Vous ne pouvez pas créer de projets. Contactez un administrateur.'
                    );
                } else {
                    $validator->errors()->add(
                        'filiale_executant',
                        'En tant que manager, au moins une des filiales (Exécutant ou Contractant) doit correspondre à votre filiale (' . $userFiliale . '). Sinon, vous ne pourrez pas voir ce projet après sa création.'
                    );
                }
            }
